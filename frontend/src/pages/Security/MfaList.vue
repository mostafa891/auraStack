...
<template>
  <AuthLayout :title="t('mfa.list_title')" :subtitle="t('mfa.list_subtitle')">
    <div class="space-y-6">
      <div class="flex items-start space-x-4">
        ...
        <div>
          <h3 class="text-sm font-semibold text-[var(--color-text)]">{{ t('mfa.app_title') }}</h3>
      ...
      <div class="pt-5 border-t border-[var(--color-border)] flex items-center justify-between">
        <div class="flex items-center space-x-2">
          <span class="text-xs text-[var(--color-text-muted)] font-medium">{{ t('mfa.status') }}</span>
          <span
            v-if="totp_active"
